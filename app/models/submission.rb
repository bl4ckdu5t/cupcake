class Submission < ActiveRecord::Base
  ...
  has_attached_file :preview, bucket: 'designscook-assets', styles: {
		...
		square: '200x200#',
		medium: '300x300>'
	}, default_url: '/default/no-preview.jpg', storage: :s3, url: ":s3_domain_url", path: "/submissions/:id/preview/:style.:extension",
	s3_host_name: 's3.amazonaws.com', s3_credentials: "#{Rails.root}/config/aws.yml"
	# Validate the attached image is image/jpg, image/png, etc
	validates_attachment_content_type :preview, :content_type => /\Aimage\/.*\Z/

	has_attached_file :project, default_url: '/default/no-preview.jpg', storage: :s3, url: ":s3_domain_url", styles: {
		thumb: '100x100#'
